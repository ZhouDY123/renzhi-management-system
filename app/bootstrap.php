...
function xlsx_rows(string $path): array { if(strtolower(pathinfo($_FILES['xlsx']['name']??'',PATHINFO_EXTENSION))!=='xlsx')throw new RuntimeException('请上传 .xlsx Excel 文件');try{$zip=new PharData($path);$xml=(string)$zip['xl/worksheets/sheet1.xml']->getContent();}catch(Throwable){throw new RuntimeException('Excel 文件无法读取，请使用系统下载的 .xlsx 模板');}$sheet=simplexml_load_string($xml);if(!$sheet)throw new RuntimeException('Excel 文件内容无效');$ns=$sheet->children('http://schemas.openxmlformats.org/spreadsheetml/2006/main');$out=[];foreach($ns->sheetData->row as $row){$cols=[];foreach($row->c as $cell){$c=$cell->children('http://schemas.openxmlformats.org/spreadsheetml/2006/main');$cols[]=trim((string)$c->is->t?:$c->v);}if($cols)$out[]=$cols;}return $out; }
