...
function xlsx_download(string $filename,array $headings,array $rows,string $sheet='数据'): never { $tmp=tempnam(sys_get_temp_dir(),'renzhi-xlsx-');@unlink($tmp);$path=$tmp.'.xlsx';$esc=fn($v)=>htmlspecialchars((string)$v,ENT_XML1|ENT_QUOTES,'UTF-8');$cell=fn($v)=>'<c t="inlineStr"><is><t>'.$esc($v).'</t></is></c>';$xml='<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData><row r="1">'.implode('',array_map($cell,$headings)).'</row>';foreach($rows as $i=>$row)$xml.='<row r="'.($i+2).'">'.implode('',array_map($cell,$row)).'</row>';$xml.='</sheetData></worksheet>';$zip=new PharData($path,0,null,Phar::ZIP);$zip['[Content_Types].xml']='<?xml version="1.0" encoding="UTF-8"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/worksheets/sheet1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/></Types>';$zip['_rels/.rels']='<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>';$zip['xl/workbook.xml']='<?xml version="1.0" encoding="UTF-8"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="'.$esc($sheet).'" sheetId="1" r:id="rId1"/></sheets></workbook>';$zip['xl/_rels/workbook.xml.rels']='<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/></Relationships>';$zip['xl/worksheets/sheet1.xml']=$xml;unset($zip);header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');header("Content-Disposition: attachment; filename*=UTF-8''".rawurlencode($filename.'.xlsx'));header('Content-Length: '.filesize($path));readfile($path);@unlink($path);exit; }
function xlsx_download_zip(string $filename,array $headings,array $rows,string $sheet='数据'): never { $tmp=tempnam(sys_get_temp_dir(),'renzhi-xlsx-');@unlink($tmp);$path=$tmp.'.xlsx';$esc=fn($v)=>htmlspecialchars((string)$v,ENT_XML1|ENT_QUOTES,'UTF-8');$cell=fn($v)=>'<c t="inlineStr"><is><t>'.$esc($v).'</t></is></c>';$xml='<?xml version="1.0" encoding="UTF-8"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData><row r="1">'.implode('',array_map($cell,$headings)).'</row>';foreach($rows as $i=>$row)$xml.='<row r="'.($i+2).'">'.implode('',array_map($cell,$row)).'</row>';$xml.='</sheetData></worksheet>';$zip=new PharData($path,0,null,Phar::ZIP);$zip['[Content_Types].xml']='<?xml version="1.0" encoding="UTF-8"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/worksheets/sheet1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/></Types>';$zip['_rels/.rels']='<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>';$zip['xl/workbook.xml']='<?xml version="1.0" encoding="UTF-8"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="'.$esc($sheet).'" sheetId="1" r:id="rId1"/></sheets></workbook>';$zip['xl/_rels/workbook.xml.rels']='<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/></Relationships>';$zip['xl/worksheets/sheet1.xml']=$xml;unset($zip);header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');header("Content-Disposition: attachment; filename*=UTF-8''".rawurlencode($filename.'.xlsx'));header('Content-Length: '.filesize($path));readfile($path);@unlink($path);exit; }
function xlsx_rows(string $path): array { if(strtolower(pathinfo($_FILES['xlsx']['name']??'',PATHINFO_EXTENSION))!=='xlsx')throw new RuntimeException('请上传 .xlsx Excel 文件');try{$zip=new PharData($path);$xml=(string)$zip['xl/worksheets/sheet1.xml']->getContent();$shared=[];if(isset($zip['xl/sharedStrings.xml'])){$strings=simplexml_load_string((string)$zip['xl/sharedStrings.xml']->getContent());foreach($strings?->xpath('//*[local-name()="si"]')?:[] as $item)$shared[]=implode('',array_map('strval',$item->xpath('.//*[local-name()="t"]')));}}catch(Throwable){throw new RuntimeException('Excel 文件无法读取，请使用系统下载的 .xlsx 模板');}$sheet=simplexml_load_string($xml);if(!$sheet)throw new RuntimeException('Excel 文件内容无效');$out=[];foreach($sheet->xpath('//*[local-name()="row"]')?:[] as $row){$cols=[];foreach($row->xpath('./*[local-name()="c"]')?:[] as $cell){$type=(string)$cell['t'];$value='';if($type==='s'){$index=(int)($cell->xpath('./*[local-name()="v"]')[0]??0);$value=$shared[$index]??'';}elseif($type==='inlineStr')$value=implode('',array_map('strval',$cell->xpath('.//*[local-name()="t"]')));else $value=(string)($cell->xpath('./*[local-name()="v"]')[0]??'');$cols[]=trim($value);}if($cols)$out[]=$cols;}if(count($out)<2)throw new RuntimeException('Excel 中没有可导入的数据，请按下载模板填写后再上传');return $out; }
