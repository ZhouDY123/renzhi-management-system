...
function initTablePagination() {
  ...
  document.querySelectorAll('.table-wrap table').forEach((table, index)=>{
    const rows=[...table.tBodies].flatMap(body=>[...body.rows]), size=isInterviewResults?10:12; if(rows.length<=size)return; let page=1; const pages=Math.ceil(rows.length/size),label=isInterviewResults?(index===0?'面试结果':'面试官评分明细'):'表格',nav=document.createElement('nav');nav.className='table-pagination';nav.setAttribute('aria-label',`${label}分页`);nav.innerHTML='<span></span><div><button type="button" class="btn secondary" data-prev>上一页</button><b></b><button type="button" class="btn secondary" data-next>下一页</button></div>';table.closest('.table-wrap').append(nav);
